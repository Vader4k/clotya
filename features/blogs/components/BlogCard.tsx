...
const BlogCard = ({ categories, intro, slug, title, date, image, tags, blogPage, style }: BlogCardProps) => {
  return (
    <div className={`w-full ${style} relative group overflow-hidden flex flex-col gap-3 items-start justify-between font-jost`}>
      <Link href={`/blogs/${slug}`} className={`block w-full h-[85%] lg:h-[70%] relative ${blogPage ? 'h-[90%] lg:h-[90%]' : ''}`}>
        <Image
          src={image}
          alt={title}
          fill
      ...
      <p className={`text-sm font-light text-gray-600 line-clamp-2 font-inter ${blogPage ? 'text-lg font-normal' : ''}`}>{intro}</p>
      {blogPage && (
        <Link href={`/blogs/${slug}`} className='flex text-lg items-center gap-3 text-gray-600 font-medium'>
          <p>Read More</p>
          <MoveRight size={16} />
        </Link>
      )}
