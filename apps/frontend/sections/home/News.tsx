...
const News = async () => {
  ...
  return (
    <section className="2xl:h-screen w-full max-w-7xl mx-auto my-6 md:my-14 py-6 md:py-14 border-y">
      <div className="flex flex-col items-center justify-center text-center">
        <h3 className="text-3xl font-medium font-jost">Our Latest News</h3>
        <p className=" font-light text-gray-600 mt-4 max-w-3xl text-xs md:text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-14 lg:gap-6 mt-10 md:mt-16">
        {blogData.blogs.map((blog) => (
          <BlogCard key={blog._id} {...blog} style=" h-[450px] sm:h-[650px] lg:h-[430px]"/>
        ))}
      </div>
    </section>
  )
