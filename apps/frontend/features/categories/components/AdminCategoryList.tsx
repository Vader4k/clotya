...
export const AdminCategoryList = () => {
  ...
  const handleEdit = async (data: CategorySchemaType) => {
    ...
    try {
      const res = await categoriesService.editCategory({
        data,
        id: String(currentCategory._id),
      });
      refetch();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-xs">
            Manage your store product categories here.
          </p>
        </div>
        <button
      ...
      {/* Error State */}
      {isError && <AdminErrorState refetch={refetch} />}
