...
export const updateProfile = async (req, res) => {
    try {
        const { name, email, phone, streetAddress, apartment, city, state, postalCode, country } = req.body;
...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: 'success', order, message: "order updated successfully" });
    } catch (error) {
        res.status(500).json({ status: 'fail', message: "internal server error" });
    }
};
